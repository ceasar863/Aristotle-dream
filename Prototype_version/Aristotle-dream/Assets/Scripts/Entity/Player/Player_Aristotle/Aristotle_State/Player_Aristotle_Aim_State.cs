...
public class Player_Aristotle_Aim_State : Player_State
{
    ...
    public Player_Aristotle_Aim_State(Player_Aristotle player_aristotle, State_Machine state_machine, string name) : base(player_aristotle, state_machine, name)
    {
    }
    ...
    public override void Enter()
    {
        ...
        ammo = player_aristotle.gameObject.GetComponentInChildren<Ammo>();
        if(has_dots==false)
            dots = Generate_Dots();
    }
    ...
    public override void Update()
    {
        ...
        if (Player_Input_System.instance.player_input.Player.Shoot.WasPressedThisFrame() && ammo.Get_Current_Bullet() != null)
        {
    ...
    private void Draw_Aim_Line(Vector2 direction)
    {
       for(int i=0; i<sum_point; i++)
        {
            var bullet = ammo.Get_Current_Bullet();
